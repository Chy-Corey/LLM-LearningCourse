...
![KV-Cache](.\image\11\KV-Cache.png)
...
我们知道，一个M\*N的矩阵可以近似成两个M\*k和k\*N矩阵的乘积（低秩分解），那如果我把一个K或者V矩阵拆成两个小矩阵的乘积，缓存的时候显存占用不就变小了吗？
...
利用矩阵的结合率，我们可以在推理的时候，提前算好 ${W^q_t}^{T}*W^{UK}_t$ ，这样在decode的时候，计算量基本没有增加（只和单个query乘了一次，而不需要和所有的缓存值重新相乘），这被称为矩阵吸收（absorb）.
