...
![LLama2](.\image\05\LLama2.png)



### 一、定义超参数

首先需要定义一些超参数，这些超参数包括**模型的大小、层数、头数、词嵌入维度、隐藏层维度**等等。这些超参数可以根据实际情况进行调整。

这里自定义一个`ModelConfig`类，来存储和记录我们的超参数，这里我们继承了`PretrainedConfig`类，这是`transformers`库中的**参数类**，我们可以通过继承这个类来方便的使用`transformers`库中的一些功能，也方便在后续导出Hugging Face模型。
...
class ModelConfig(PretrainedConfig):
    ...
    def __init__(
            self,
            dim: int = 768, # 模型维度
            n_layers: int = 12, # Transformer的层数，在LLAMA中表示Decoder的层数
            n_heads: int = 16, # 注意力机制的头数，即多头注意力机制的查询头（Query Heads）数量
            n_kv_heads: int = 8, # 键值头的数量，即16/8个query共享一个key-value
...
我们来看一下其中的一些超参数的含义，比如`dim`是模型维度，`n_layers`是Transformer的层数，`n_heads`是注意力机制的头数，`vocab_size`是词汇表大小，`max_seq_len`是输入的最大序列长度等等。上面的代码中也对每一个参数做了详细的注释，在后面的代码中我们会根据这些超参数来构建我们的模型。`multiple_of` 是一个用于**约束模型隐藏层维度（Hidden Dimension）的“对齐参数”**。这是 `multiple_of` 最本质的作用。在 GPU 等硬件上进行大规模矩阵运算时，如果张量的维度是某个特定数值（通常是 2 的幂，如 64、128、256 等）的整数倍，硬件的并行计算单元就能得到最充分的利用，内存读取和计算效率会达到最高。因此，`multiple_of` 强制要求模型前馈网络（FFN）的中间隐藏层维度必须是这个数值的整数倍，从而避免因为维度“不整齐”而产生的计算资源浪费。

### 二、构建 RMSNorm（Root Mean Square Normalization）

`RMSNorm`可以用如下的数学公式表示：
$$
RMSNorm(x)=\frac{x}{\sqrt{\frac{1}{n}\sum_{i=1}^{n}x_{i}^2+\epsilon}}\cdot \gamma
$$
其中：
...
这种归一化有助于通过确保**权重的数值规模不会变得过大或过小来稳定学习过程**，这在具有许多层的深度学习模型中特别有用。

> 为什么对隐状态的归一化，能够保证权重的稳定？
>
> **1. 反向传播的“连锁反应”：约束梯度，防止权重更新失控**
> 在神经网络的反向传播过程中，权重的更新量（梯度）是由**上一层的激活值（隐状态）**决定的。
>
> - **没有归一化时**：如果某一层输出的隐状态数值非常大（比如达到了几百、几千），在计算梯度时，这个巨大的数值会直接参与乘法运算。这会导致计算出的权重梯度也变得极其巨大，导致权重在一次更新中发生剧烈的“跳跃”，模型训练就会震荡甚至崩溃（梯度爆炸）。
> - **有了 RMSNorm 后**：它强制把每一层的隐状态都缩放到了一个相对平稳、合理的尺度（通常在 1 附近）。当反向传播计算梯度时，由于隐状态被“稳”住了，算出来的权重梯度也会保持在一个合理的范围内。梯度稳定了，权重的更新步伐（步长）自然就稳了，权重规模也就不会忽大忽小。
>
> **2. 隐式的学习率调节：权重大时自动“刹车”**
> RMSNorm 具有一种非常巧妙的特性，可以看作是为权重引入了一种**“隐式的学习率调节机制”**。
>
> - 在训练过程中，如果某个权重参数的范数（规模）变得过大，由于 RMSNorm 的缩放特性，反向传播回来的梯度反而会自动变小。
> - **实际效果**：这意味着当权重已经很大时，RMSNorm 会自动让它的更新步子变小（相当于自动降低了该权重的学习率），防止它继续无限制地膨胀。这种机制让模型能够自适应地维持权重的平衡。
...
可以用下面的代码来对`RMSNorm`模块进行测试，可以看到代码最终输出的形状为`torch.Size([1, 50, 768])`，与我们输入的形状一致，说明模块的实现是正确的，归一化并不会改变输入的形状。
...
<img src=".\image\05\GQA.png" alt="GQA" style="zoom:27%;" />

分组查询注意力（Grouped-Query Attention，简称 GQA）是现代大语言模型（如 LLaMA 3、Qwen 等）中广泛采用的一种注意力机制优化方案。

简单来说，GQA 是一种**在“模型性能”和“推理效率”之间取得完美平衡**的折中设计。它的核心思想是：保留多个查询头（Query Heads），但让多个查询头**分组共享**同一组键（Key）和值（Value）头。（这个机制在本章之前出现过，但没有具体解释）

我们可以从以下三个维度来拆解：

**1. GQA 是如何“分组”的？** 在标准的 Transformer 架构中，注意力机制经历了三个阶段的演变：

- **多头注意力 (MHA)**：最传统的方式。每个查询头（Q）都有自己专属的键（K）和值（V）。比如 32 个 Q 头，就必须配备 32 组 K 和 V。这虽然表达能力最强，但计算和存储开销巨大。
- **多查询注意力 (MQA)**：最极端的省流方式。所有 32 个 Q 头，全部共享**同一组** K 和 V。虽然速度最快，但模型“看”信息的角度太少，导致表达能力下降，效果往往不如人意。
- **分组查询注意力 (GQA)**：折中的最优解。比如 32 个 Q 头，分成 4 个组，每组 8 个 Q 头共享**同一组** K 和 V。这样既保留了多角度的查询能力，又大幅减少了 K 和 V 的数量。

**2. 为什么现代大模型都要用 GQA？** GQA 最大的价值在于**显著降低推理时的显存占用和带宽压力**，尤其是在处理长文本时。

在模型生成文字（推理）的过程中，需要把历史内容的 Key 和 Value 一直保存在显存里（也就是常说的 **KV Cache**）。

- 如果用 **MHA**，KV Cache 的体积非常大，随着文本变长，显存迅速爆满，生成速度也会因为频繁读取显存而变慢。
- 换成 **GQA** 后，由于 K 和 V 的数量成倍减少（例如从 32 组减到 4 组），**KV Cache 的体积直接缩小了 8 倍**！这不仅节省了宝贵的显存，还大幅降低了内存带宽的占用，让模型在生成长文时速度更快、更流畅。

目前主流的大模型（如 LLaMA 2/3、Qwen、Mistral 等）之所以普遍采用 GQA，就是因为它用最小的性能代价，换来了推理效率的巨大提升。

#### 1. repeat_kv

在 LLaMA2 模型中，我们需要将**键和值的维度扩展到和查询的维度一样**，这样才能进行注意力计算。我们可以通过如下代码实现`repeat_kv`：
...
#### 2. 旋转嵌入（Rotary Position Embedding，RoPE）

接着我们来实现旋转嵌入，旋转嵌入是 LLaMA2 模型中的一个重要组件，它可以**为注意力机制提供更强的上下文信息**，从而提高模型的性能。

RoPE是对标于transformer的位置编码的，传统的绝对位置编码（如早期的 GPT、BERT）是把位置信息直接**加**到词向量上。而 RoPE 的做法非常巧妙，它不改变向量的大小，而是根据词在句子中的位置，对向量进行一定角度的**旋转**。

在 LLaMA 模型中，RoPE 的应用有两个非常关键的细节：

- **只对 Q 和 K 生效**：RoPE 只会作用于自注意力机制中的查询向量（Query）和键向量（Key），而不会去旋转值向量（Value）。因为注意力机制的核心是计算 Q 和 K 的匹配度（即“谁应该关注谁”），这需要位置信息；而 V 承载的是词本身的语义内容，不需要被位置扭曲。
- **多维空间的成对旋转**：在实际计算中，模型会将高维向量（比如 128 维）切分成多个两两一组的“小二维平面”。每个平面会使用不同的旋转频率（角度变化速度）进行独立旋转，从而在极高维的空间中编码极其丰富的位置信息。
...
### 二、Character-based Tokenizer

**Character-based Tokenizer** 将文本中的**每个字符视为一个独立的 token**。这种方法能非常精细地处理文本，适用于处理拼写错误、未登录词或新词。由于每个字符都是一个独立的 token，因此这种方法可以捕捉到非常细微的语言特征。这对于一些特定的应用场景，如生成式任务或需要处理大量未登录词的任务，特别有用。但是，这种方法也会导致 token 序列变得非常长，增加了模型的计算复杂度和训练时间。此外，字符级的分割可能会丢失一些词级别的语义信息，使得模型难以理解上下文。
...
**Subword Tokenizer** 介于词和字符之间，能够更好地平衡分词的细粒度和处理未登录词的能力。Subword Tokenizer 的关键思想是**将文本分割成比单词更小的单位，但又比字符更大，这样既能处理未知词，又能保持一定的语义信息**。常见的子词分词方法包括 BPE、WordPiece 和 Unigram。
...
**WordPiece** 是另一种基于子词的分词方法，最初用于谷歌的 BERT 模型。与 BPE 类似，WordPiece 通过**最大化子词序列的似然函数来生成词典，但在合并子词时更注重语言模型的优化**。WordPiece 会优先选择能够最大化整体句子概率的子词，使得分词结果在语言模型中具有更高的概率。
...
**Unigram** 分词方法基于概率模型，通过**选择具有最高概率的子词来分割文本**。Unigram 词典是通过训练语言模型生成的，可以处理多种语言和不同类型的文本。Unigram 模型会为每个子词分配一个概率，然后根据这些概率进行最优分割。
...
![PretrainDateset](E:\Note\HuaweiLearningCourse\LLM-LearningCourse\image\05\PretrainDateset.png)
...
[![alt text](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/5-images/sftdataset.png)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/5-images/sftdataset.png)
...
训练时一次前向传播，是对整条完整序列并行算出所有位置的预测，不是只算一个 token，也不是“生成到 EOS 才停”的自回归循环。

训练时（Train）

- 输入：完整的长序列（例如 1024 个 token，含 BOS/EOS）。
- 前向：一次前向 = 整串并行计算，每个位置同时预测“下一个 token”的概率分布。- 输入： [t₀, t₁, t₂, ..., tₙ] 
- 输出： [logits₁, logits₂, ..., logitsₙ] （每个位置对应预测下一个 token 的 logits）
- 损失：一次性算全序列的交叉熵，并行更新参数。
- 一句话：训练是全序列并行预测，不是逐 token 生成。

推理时（Generate）

- 原生逻辑：一次前向只生成 1 个 token，自回归循环直到 EOS 或最大长度。1. 输入 prompt → 前向 → 取最后一位 logits → 采样出 1 个新 token
2. 把新 token 拼回输入 → 再前向 → 再 1 个 token
3. 循环直到 EOS 或 max_len
- 工程优化：用 KV Cache 缓存历史 key/value，避免每次重复计算，加速自回归过程，但逻辑上仍是逐 token 串行生成。

关键区别

- 训练：一次前向 = 全序列所有位置的预测（并行）
- 推理：一次前向 = 1 个 token，多次循环到 EOS（串行）
...
[happy-llm/docs/chapter5/第五章 动手搭建大模型.md at main · datawhalechina/happy-llm](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter5/第五章 动手搭建大模型.md)